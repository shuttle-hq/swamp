...
use syn::{
    parse2, parse_macro_input, punctuated::Punctuated, token::Comma,
    AngleBracketedGenericArguments, Attribute, Data, DataEnum, DataStruct, DeriveInput, Field,
    Fields, FieldsNamed, FieldsUnnamed, GenericArgument, GenericParam, Generics, Lit, LitStr, Meta,
    MetaList, MetaNameValue, NestedMeta, Path, PathArguments, Type, TypeParam, TypePath, Variant,
};
    ...
    let (label_output, parse_label_output) = match input.data {
        Data::Struct(DataStruct {
            ...
            output.unwrap_or_else(|| {
                ...
                if named.len() != 1 {
                    panic!("struct has no `name` field but has more than one field or no field")
                }
...
pub fn derive_block(input: TokenStream) -> TokenStream {
    ...
    match input.data {
        Data::Struct(DataStruct {
            ...
            // only passthrough structs (such as prost oneofs) are allowed
            if named.len() != 1 {
                panic!("block structs can only have one field pointing to a nested label")
            }
            let field = named.pop().unwrap().into_value();
            let field_ident = field.ident.unwrap();
            let field_type = extract_type_path(field.ty);
